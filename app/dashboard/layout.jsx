...
export default function DashboardShell({ children }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--sat-bg)' }}>
      {/* TOPBAR */}
      <header style={{ position:'fixed', top:0, left:0, right:0, zIndex:30, height:52, background:'var(--sat-surface)', borderBottom:'1px solid var(--sat-border)', display:'flex', alignItems:'center', gap:14, padding:'0 16px' }}>
        <button onClick={() => setSideOpen(true)} style={{ background:'none', border:'none', cursor:'pointer', padding:6, display:'flex', flexDirection:'column', gap:5, flexShrink:0 }}>
        ...
        {role === 'raven' ? (
          <span style={{ fontSize:9, fontFamily:'DM Mono,monospace', letterSpacing:'0.1em', color:'#f59e0b', background:'#f59e0b10', padding:'3px 8px', borderRadius:3, border:'1px solid #f59e0b28' }}>◆ RAVEN</span>
        ) : (
          <span style={{ width:7, height:7, borderRadius:'50%', background: role==='admin'?'var(--sat-info)':'var(--sat-accent)', boxShadow:`0 0 8px ${role==='admin'?'var(--sat-info)':'var(--sat-accent)'}` }} />
        )}
      </header>
