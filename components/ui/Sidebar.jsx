  ...
  return (
    <>
      ...
      {open && (
        <div
          onClick={onClose}
          style={{ position:'fixed', inset:0, zIndex:40, background:'#00000060', backdropFilter:'blur(2px)' }}
        />
      )}
      ...
      <aside style={{
        ...
        {/* Header */}
        <div style={{ padding:'16px 18px', borderBottom:'1px solid var(--sat-border)', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ width:7, height:7, borderRadius:'50%', background:'var(--sat-accent)', boxShadow:'0 0 10px var(--sat-accent)' }} />
              <span style={{ fontFamily:'DM Mono,monospace', fontSize:11, color:'var(--sat-accent)', letterSpacing:'0.12em' }}>SATÉLITE ON</span>
            </div>
            <div style={{ marginTop:6, display:'flex', gap:6 }}>
              {raven && (
                <span style={{ fontSize:9, fontFamily:'DM Mono,monospace', color:'var(--sat-warn)', background:'#f59e0b10', padding:'2px 7px', borderRadius:3, border:'1px solid #f59e0b28' }}>◆ RAVEN</span>
              )}
              {admin && !raven && (
                <span style={{ fontSize:9, fontFamily:'DM Mono,monospace', color:'var(--sat-info)', background:'#3b82f610', padding:'2px 7px', borderRadius:3, border:'1px solid #3b82f628' }}>ADMIN</span>
              )}
              {role === 'empleado' && (
                <span style={{ fontSize:9, fontFamily:'DM Mono,monospace', color:'var(--sat-dim)', background:'#ffffff08', padding:'2px 7px', borderRadius:3, border:'1px solid var(--sat-border)' }}>EMPLEADO</span>
              )}
            </div>
          </div>
        ...
        {user?.nombre && (
          <div style={{ padding:'10px 18px', borderBottom:'1px solid var(--sat-border)' }}>
            ...
            <p style={{ margin:'2px 0 0', fontSize:11, color:'var(--sat-dim)' }}>{user.email}</p>
          </div>
        )}
        ...
        <div style={{ padding:'14px 16px', borderTop:'1px solid var(--sat-border)', flexShrink:0 }}>
          <button onClick={signOut} style={{ width:'100%', padding:'9px 0', borderRadius:7, fontSize:12, background:'transparent', border:'1px solid var(--sat-border)', color:'var(--sat-dim)', cursor:'pointer' }}>
            Cerrar sesión
          </button>
        </div>
      </aside>
